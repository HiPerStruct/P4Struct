...
from engine import finite_element_analysis
from engine import optimization_analysis
...
class _CreateFiniteElementAnalysisTaskManager(QtWidgets.QDialog):
    ...
    def __slotSubmitTask(self) -> None:
        ...
        if self.__task_process_dict == {}:
            ...
            if task_type == 'FEM':
                ins_task_process = multiprocessing.Process(target=finite_element_analysis.intoFEAProcess,name=task_name,args=(task_file_full_name,self.__ins_task_process_queen,),)
            else:
                pass
...
class _CreateOptimizationTaskManager(QtWidgets.QDialog):
    ...
    def __slotSubmitTask(self) -> None:
        if self.__task_process_dict == {}:
            ...
            task_type = ins_task_table.item(ins_task_table.currentRow(),2).text()
            ins_task_process = multiprocessing.Process(target=optimization_analysis.intoOPTProcess,name=task_name,args=(task_type,task_file_full_name,self.__ins_task_process_queen,),)
            self.__task_process_dict[task_name] = ins_task_process
            ins_task_process.start()
