...
class P4SModelVisualWindow(QtWidgets.QMdiSubWindow):
    ...
    def addCoordinateSystemToCurrentViewport(self, in_coordinate_system_name:str, in_coordinate_system_info:dict) -> None:
        self.widget().addCoordinateSystemToCurrentRenderer(in_coordinate_system_name, in_coordinate_system_info)
...
class _ModelVisualizationInteractor(QVTKRenderWindowInteractor.QVTKRenderWindowInteractor):
    ...
    def addCoordinateSystemToCurrentRenderer(self, in_coordinate_system_name:str, in_coordinate_system_info:dict) -> None:
        ...
        ins_coordinate_system_transformer.Translate(in_coordinate_system_info['origin'])
        ins_coordinate_system_transformer.RotateWXYZ(*in_coordinate_system_info['orientation'])
        ins_coordinate_system_actor.SetUserTransform(ins_coordinate_system_transformer)
        if in_coordinate_system_info['type'] == 1:
...
class P4SResultVisualWindow(QtWidgets.QMdiSubWindow):
    ...
    def changeVariableComponent(self, in_step_name:str, in_frame_name:str, in_component_name:str) -> None:
        self.widget().changeVariableComponentOfCountorRenderer(in_step_name, in_frame_name, in_component_name)
...
class _ResultVisualizationInteractor(QVTKRenderWindowInteractor.QVTKRenderWindowInteractor):
    ...
    def __initializeContourRenderer(self) -> None:
        ...
        for instance_name,instance_location_array in self.__ins_database_pointer['Mesh']['Instances'].items():
            ...
            ins_instance_actor.SetMapper(ins_instance_mapper)
            ins_instance_actor.PickableOn()
            ins_instance_actor_prop = ins_instance_actor.GetProperty()
            ins_instance_actor_prop.LightingOn()
            ins_instance_actor_prop.SetAmbient(1.0)
            ins_instance_actor_prop.SetDiffuse(0.0)
            ins_instance_actor_prop.SetSpecular(0.0)
    ...
    def changeColorNumberOfCountorRenderer(self, in_map_name:str, in_color_number:int) -> None:
        for ins_actor in self.__ins_contour_renderer.GetActors2D():
            if ins_actor.GetObjectName() == 'scalar-bar':
                ins_actor.SetNumberOfLabels(in_color_number+1)
                ins_scalar_bar_lookup_table = ins_actor.GetLookupTable()
                break
            else:
                continue
